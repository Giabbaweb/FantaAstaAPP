...
CREATE TABLE `__new_auction_events` (
	...
	`before_auction_session_team_id` text,
	`before_player_id` text,
	`before_amount` integer,
	`before_contract_year` integer,
	`after_auction_session_team_id` text,
	`after_player_id` text,
	`after_amount` integer,
	`after_contract_year` integer,
	...
	FOREIGN KEY (`before_auction_session_team_id`) REFERENCES `auction_session_teams`(`id`) ON UPDATE no action ON DELETE restrict,
	FOREIGN KEY (`before_player_id`) REFERENCES `players`(`id`) ON UPDATE no action ON DELETE restrict,
	FOREIGN KEY (`after_auction_session_team_id`) REFERENCES `auction_session_teams`(`id`) ON UPDATE no action ON DELETE restrict,
	FOREIGN KEY (`after_player_id`) REFERENCES `players`(`id`) ON UPDATE no action ON DELETE restrict,
	CONSTRAINT "auction_events_shape_consistency" CHECK((
        (
          ...
          AND "__new_auction_events"."before_auction_session_team_id" IS NULL
          AND "__new_auction_events"."before_player_id" IS NULL
          AND "__new_auction_events"."before_amount" IS NULL
          AND "__new_auction_events"."before_contract_year" IS NULL
          AND "__new_auction_events"."after_auction_session_team_id" IS NULL
          AND "__new_auction_events"."after_player_id" IS NULL
          AND "__new_auction_events"."after_amount" IS NULL
          AND "__new_auction_events"."after_contract_year" IS NULL
        ...
        OR
        (
          "__new_auction_events"."event_type" = 'TECHNICAL_ROSTER_CORRECTION'
          AND "__new_auction_events"."auction_call_id" IS NULL
          ...
          AND "__new_auction_events"."actor_name" IS NOT NULL
          AND "__new_auction_events"."actor_role" IS NOT NULL
          AND "__new_auction_events"."comment" IS NOT NULL
          AND length(trim("__new_auction_events"."comment")) > 0
          AND "__new_auction_events"."manual_assignment_reason" IS NULL
          AND "__new_auction_events"."before_auction_session_team_id" IS NOT NULL
          AND "__new_auction_events"."before_player_id" IS NOT NULL
          AND "__new_auction_events"."before_amount" IS NOT NULL
          AND "__new_auction_events"."before_contract_year" IS NOT NULL
          AND "__new_auction_events"."after_auction_session_team_id" IS NOT NULL
          AND "__new_auction_events"."after_player_id" IS NOT NULL
          AND "__new_auction_events"."after_amount" IS NOT NULL
          AND "__new_auction_events"."after_contract_year" IS NOT NULL
          AND "__new_auction_events"."suspension_reason" IS NULL
        )
        OR
        (
	...
	CONSTRAINT "auction_events_before_amount_positive" CHECK("__new_auction_events"."before_amount" IS NULL OR "__new_auction_events"."before_amount" > 0),
	CONSTRAINT "auction_events_after_amount_positive" CHECK("__new_auction_events"."after_amount" IS NULL OR "__new_auction_events"."after_amount" > 0),
	CONSTRAINT "auction_events_before_contract_year_range" CHECK("__new_auction_events"."before_contract_year" IS NULL OR "__new_auction_events"."before_contract_year" BETWEEN 1 AND 3),
	CONSTRAINT "auction_events_after_contract_year_range" CHECK("__new_auction_events"."after_contract_year" IS NULL OR "__new_auction_events"."after_contract_year" BETWEEN 1 AND 3),
...
INSERT INTO `__new_auction_events`("id", "auction_session_id", "auction_call_id", "event_type", "auction_session_team_id", "player_id", "amount", "credits_before", "credits_after", "contract_year", "actor_name", "actor_role", "comment", "manual_assignment_reason", "before_auction_session_team_id", "before_player_id", "before_amount", "before_contract_year", "after_auction_session_team_id", "after_player_id", "after_amount", "after_contract_year", "suspension_reason", "created_at") SELECT "id", "auction_session_id", "auction_call_id", "event_type", "auction_session_team_id", "player_id", "amount", "credits_before", "credits_after", "contract_year", "actor_name", "actor_role", "comment", "manual_assignment_reason", NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, "suspension_reason", "created_at" FROM `auction_events`;--> statement-breakpoint
